...
pg.submit.onclick = function() {
    if (
        ...
        window.alert('You must enter a TEAM NUMBER, COLOR, POSITION, and a MATCH');
    }
};
...
function write(match) {
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i].match === match.match && data[i].team === match.team) {
            if (confirm('Do you want to replace the OLD data for this match?')) {
                data.splice(i--, 1);
            }
        }
    }
...
pg.reset.onclick = function() {
    if (window.confirm('Reset inputs?')) resetInputs();
};
